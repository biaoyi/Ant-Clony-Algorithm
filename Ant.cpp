...
int Ant::chooseCity()
{
    ...
    if(total > 0.0)
    {
        ...
        for(int i = 0; i < N_CITY; i++)
        {
            //        if(std::find(allowed.begin(),allowed.end(),i) != allowed.end())                 //如果允许去
            if(allowed[i] == 1)                 //如果允许去
            {
                temp -= prob[i];
                if(temp < 0.0)                     //轮盘停止
...
//    tabuk.push_back(select);
//    allowed.erase(std::find(allowed.begin(),allowed.end(),select));
...
//    distance += city_distance[tabuk[N_CITY - 1]][tabuk[0]];
    n = travel[0];
    distance += city_distance[m][n];
}
